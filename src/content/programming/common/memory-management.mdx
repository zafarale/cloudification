...
```mermaid
flowchart TD
    C["<b>C (1972)</b><br/>Manual: malloc/free"] --> Java
    C --> Rust
    
    Java["<b>Java (1995)</b><br/>Runtime Solution"]
    Rust["<b>Rust (2015)</b><br/>Compile-time Solution"]
    
    Java --> J1["✓ No manual free()"]
    Java --> J2["✓ No use-after-free"]
    Java --> J3["✗ Runtime overhead"]
    Java --> J4["✗ Unpredictable pauses"]
    
    Rust --> R1["✓ No manual free()"]
    Rust --> R2["✓ No use-after-free"]
    Rust --> R3["✓ Zero runtime overhead"]
    Rust --> R4["✗ Steeper learning curve"]
    
    style C fill:#f9f,stroke:#333
    style Java fill:#f96,stroke:#333
    style Rust fill:#b7410e,stroke:#333,color:#fff
```
...
flowchart TB
    subgraph Stack["Call Stack"]
        direction TB
        main["<b>main()</b><br/>x = 5<br/>y = 10"]
        main --> add["<b>add(x, y)</b><br/>a = 5 (copy)<br/>b = 10 (copy)<br/>result = 15"]
        add -.->|"returns 15<br/>frame popped"| freed["Memory freed instantly"]
    end
    
    style main fill:#4a9eff,stroke:#333,color:#fff
    style add fill:#66b3ff,stroke:#333
    style freed fill:#90EE90,stroke:#333
```
...
flowchart LR
    ...
    style Stack fill:#e6f3ff,stroke:#333
    style Heap fill:#fff0e6,stroke:#333
```
...
flowchart LR
    subgraph Young["Young Generation"]
        direction TB
        Eden["Eden Space<br/><i>new objects</i>"] -->|"minor GC<br/>(fast)"| Survivor["Survivor S0/S1"]
        Survivor -->|"survived 1+ GCs"| Promote["Promote"]
    end
    
    subgraph Old["Old Generation"]
        Tenured["Tenured Space<br/><i>long-lived objects</i>"]
    end
    
    Promote -->|"after threshold"| Tenured
    Tenured -.->|"major GC (slow)"| Tenured
    
    style Eden fill:#90EE90,stroke:#333
    style Survivor fill:#FFD700,stroke:#333
    style Tenured fill:#FF6B6B,stroke:#333
```
