...
<KeyPoints>
- How identities for humans and workloads differ — and why that distinction matters
- How to design least-privilege access boundaries by role and scope
- How to trace the policy evaluation path when access is unexpectedly denied or allowed
- How to write well-scoped IAM policies instead of overly broad permissions
- The most dangerous IAM anti-patterns and why they fail
</KeyPoints>
...
```mermaid
flowchart TD
    accTitle: The Core Problem IAM Solves
    accDescr: Flowchart (td) showing API Request Model, user / service / app, read / write / delete
    ...
    subgraph Request["API Request Model"]
        Who["fa:fa-user Who?<br/>Identity<br/>(user / service / app)"]:::step
        What["fa:fa-key Can do what?<br/>Permission<br/>(read / write / delete)"]:::step
        Where["fa:fa-box To what?<br/>Resource<br/>(bucket / VM / database)"]:::step
        When["fa:fa-clock When?<br/>Condition<br/>(MFA / IP / time / tag)"]:::step
    end

    Who --> What --> Where --> When --> Decision{"Evaluation"}:::decesion
    Decision -->|"Allow matches, no deny"| Allow["fa:fa-check Allowed"]:::ok
    Decision -->|"Deny or no allow"| Deny["fa:fa-xmark Denied"]:::fail
```
...
```mermaid
flowchart TD
    accTitle: Identity Types
    accDescr: Flowchart (td) showing Human Identities, User Individual person Logs in interactively, Group Collection of users Shared permissions
    classDef step    fill:none,stroke:#a855f7,color:#a855f7,rx:8,ry:8
    classDef machine fill:none,stroke:#22c55e,color:#22c55e,rx:8,ry:8
    linkStyle default stroke-width:2.5px

    subgraph Human["Human Identities"]
        User["fa:fa-user User<br/>Individual person<br/>Logs in interactively"]:::step
        Group["fa:fa-users Group<br/>Collection of users<br/>Shared permissions"]:::step
    end

    subgraph Machine["Machine / Workload Identities"]
        SA["fa:fa-robot Service Account<br/>Managed Identity<br/>No stored password"]:::machine
        Role["fa:fa-user-tag Role<br/>Assumable identity<br/>Time-limited credentials"]:::machine
    end
...
Never hardcode a user's access key or password in application code, a container image, or a CI configuration. 
Use managed/service identities — they authenticate automatically without storing credentials anywhere.
</Callout>
...
A **permission** is a single allowed (or denied) action: 
- `s3:GetObject`, 
- `Microsoft.Compute/virtualMachines/start/action`.

A **policy** is a structured document that bundles permissions together and specifies;
- who they apply to, 
- on what resources, and 
...
flowchart LR
    ...
    accDescr: Flowchart (lr) showing What does this identity ACTUALLY do, Minimum permissions for that task, Smallest resource scope needed
    classDef step fill:none,stroke:#a855f7,color:#a855f7,rx:8,ry:8
    classDef ok   fill:none,stroke:#22c55e,color:#22c55e,rx:8,ry:8
    linkStyle default stroke-width:2.5px
...
```mermaid
flowchart TD
    accTitle: Access Evaluation — How Cloud IAM Decides
    accDescr: Flowchart (td) showing API Request, 1 Identify calling identity, 2 Explicit DENY policy match
    classDef step fill:none,stroke:#a855f7,color:#a855f7,rx:8,ry:8
    classDef ok   fill:none,stroke:#22c55e,color:#22c55e,rx:8,ry:8
    classDef fail fill:none,stroke:#ef4444,color:#ef4444,rx:8,ry:8
    ...
    Granted["fa:fa-check Access Granted"]:::ok
    Denied["fa:fa-xmark Access Denied"]:::fail
...
flowchart LR
    ...
    accDescr: Flowchart (lr) showing Username Password, Second Factor TOTP code / hardware key, Authenticated
    classDef step fill:none,stroke:#a855f7,color:#a855f7,rx:8,ry:8
    classDef ok   fill:none,stroke:#22c55e,color:#22c55e,rx:8,ry:8
    classDef fail fill:none,stroke:#ef4444,color:#ef4444,rx:8,ry:8
    linkStyle default stroke-width:2.5px

    Login["fa:fa-key Username + Password"]:::step
    MFA["fa:fa-mobile Second Factor<br/>TOTP code / hardware key"]:::step
    Access["fa:fa-check Authenticated"]:::ok
    Fail["fa:fa-xmark Rejected"]:::fail
...
IAM itself is **free on all three major cloud providers** — you can create unlimited policies, roles, groups, and service accounts at no charge. Costs come from advanced identity features layered on top.

| Feature | AWS | Azure | GCP |
|---|---|---|---|
| **Core IAM** | Free | Free (Entra ID Free tier) | Free |
| **SSO / Enterprise federation** | IAM Identity Center: free | Entra ID P1: $6/user/month (Conditional Access + SSO) | Cloud Identity: free (limited); Google Workspace for advanced |
| **PIM / Just-in-Time access** | No built-in PIM | Entra ID P2: $9/user/month (Privileged Identity Management) | No native PIM; use IAM Conditions + custom workflows |
| **Hardware MFA tokens** | OATH TOTP: free; hardware keys: ~$25–65 one-time | FIDO2/hardware keys: ~$25–65 | FIDO2/hardware keys: ~$25–65 |
| **Secrets storage** | Secrets Manager: $0.40/secret/month + $0.05/10k ops | Key Vault: $0.03/10k ops; HSM key: $1.00/key/month | Secret Manager: $0.06/version/month + $0.03/10k ops |
| **Access reviews / audit** | IAM Access Analyzer: free | Entra ID P2 required for full access reviews | IAM Recommender: free |

<Callout type="warning" title="Entra ID P2 Licensing is Per-User and Required for PIM">
Azure **Privileged Identity Management (PIM)** — the feature that enables just-in-time role elevation — requires Entra ID P2 at $9/user/month per eligible administrator. In a 50-person team where 10 people need PIM-managed elevated access, that is $90/month for those 10 users alone. Budget for P2 licences when designing an Azure RBAC strategy that includes time-limited privileged access.
</Callout>

<Callout type="tip" title="Use SSM Parameter Store Instead of Secrets Manager for Non-Rotating Config">
AWS Secrets Manager at $0.40/secret/month is the right choice when you need automatic rotation via Lambda integration. For static configuration values and feature flags that don't rotate, **SSM Parameter Store Standard** is free with no limits on standard-tier parameters. At 100 secrets, the monthly cost difference is $40 — significant for small teams or high parameter counts.
</Callout>
...
```mermaid
architecture-beta
    accTitle: IAM Architecture Overview
    accDescr: Architecture diagram showing cloud, Cloud Account, internet
