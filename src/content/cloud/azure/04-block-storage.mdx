...
│                                                                   │
│  ┌──────────────────────┐    ┌─────────────────────────────────┐  │
│  │   Temporary Disk     │    │       Managed Disk              │  │
│  │                      │    │                                 │  │
│  │  Local SSD on host   │    │  Azure-managed block storage    │  │
│  │  Fast, low latency   │    │  Persists across restarts       │  │
│  │  Lost on dealloc/    │    │  AZ-scoped or ZRS option        │  │
│  │  resize/redeploy     │    │  Snapshot supported             │  │
│  │  (/dev/sdb → /mnt)   │    │  Multiple SKUs                  │  │
│  └──────────────────────┘    └─────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────┘
```
...
```
ReadOnly caching flow:
  First read:   VM → host cache miss → Azure storage → populate cache
  Second read:  VM → host cache hit  → return immediately (sub-ms)
  Write:        VM → bypass cache → Azure storage directly
```
...
```
Azure VM
  ├── OS Disk (P10/P15) – ReadOnly cache – OS and binaries only
  ├── Data Disk (P30)   – ReadOnly cache – application data
  └── Log Disk  (P30)   – None cache     – database WAL / transaction log
```
